...
export const hitosPageFraude = async (req, res) => {
  const user = req.user;
  ...
  try {
    ...
    const resultHitos = await axios.post(
      "http://localhost:8000/api/fraudes/hitosFraude",
      {
        id: req.params.id,
      }
    );
    ...
    res.render("admin/fraudes/hitosFraude", { user, datos });
  } catch (error) {
    res.redirect("/admin/fraudes");
  }
};
export const addPageHitoFraude = async (req, res) => {
  const user = req.user;
  ...
  try {
    ...
    res.render("admin/hitos/add", { user, datos });
  } catch (error) {
    res.redirect("/admin/fraudes");
  }
};
...
export const insertHitoFraude = async (req, res) => {
  ...
  try {
    ...
    let msg = "No se ha podido crear el hito. Verifique los datos introducidos";

    if (error.response.data.errorNum === 20100) {
      msg = "El hito ya existe.";
    }
    ...
    res.render(`admin/fraudes/hitos/${idFraude}`, {
      user,
      datos,
      alerts: [{ msg }],
    });
