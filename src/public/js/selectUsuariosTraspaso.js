  ...
  myList.map(element => {
    ...
    cell.innerHTML = `<ul class="dots-menu">
      <li class="nav-item drop-right">
        ...
        <ul>
          <li class="nav-item">
            <a href="/admin/traspasos/calendarios/usuarios/${element.IDUSUA}?part=${getCookie('filtro')}" class="nav-link">
              <svg xmlns="http://www.w3.org/2000/svg" class="icon icon-inline me-2" width="24" height="24" viewBox="0 0 24 24" stroke="currentColor" fill="none" stroke-linecap="round" stroke-linejoin="round">
...
const createPages = () => {
  ...
  if (hasPrevUsers) {
    str += "<li class='page-item previous no'><a href='/admin/traspasos?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarUserBox').value + "&dir=prev' class='nav-link'>&#9664 Anterior</a>";
  } else {
    str += "<li><a href='#' class='nav-link disabled'>&#9664 Anterior</a>";
  }

  if (hasNextUsers) {
    str += "<li class='page-item next no'><a href='/admin/traspasos?cursor=" + JSON.stringify(cursor) + "&part=" + document.getElementById('buscarUserBox').value + "&dir=next' class='nav-link'>Siguiente &#9654</a>";
  } else {
